                    ...
                    <form class="form form-vertical" action="<?= ($edit) ? base_url('dashboard/attraction/update') . '/' . $data['id'] : base_url('dashboard/attraction'); ?>" method="post" onsubmit="checkRequired(event)" enctype="multipart/form-data">
                        <div class="form-body">
                            ...
                            <div class="form-group mb-4">
                                ...
                                <input type="text" id="name" class="form-control" name="name" placeholder="Attraction Name" value="<?= ($edit) ? $data['name'] : old('name'); ?>" required autocomplete="off">
                            </div>
                            <div class="form-group mb-4">
                                <label for="type" class="mb-2">Attraction Type</label>
                                <input type="text" id="type" class="form-control" name="type" placeholder="Attraction Type" value="<?= ($edit) ? $data['type'] : old('type'); ?>" required autocomplete="off">
                            </div>
                            <div class="form-group mb-4">
...
<script>
    ...
    <?php if ($edit && count($data['gallery']) > 0) : ?>
        pond.addFiles(
            <?php foreach ($data['gallery'] as $g) : ?> `<?= base_url('media/photos/attraction/' . $g); ?>`,
            <?php endforeach; ?>
        );
    <?php endif; ?>
    pond.setOptions({
