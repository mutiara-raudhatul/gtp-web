<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>System Error - Desa Wisata GTP Ulakan</title>
    <link rel="stylesheet" href="<?= base_url('assets/css/main/app.css'); ?>">
    <link rel="stylesheet" href="<?= base_url('assets/css/pages/error.css'); ?>">
    <link rel="shortcut icon" href="<?= base_url('media/icon/favicon.svg'); ?>" type="image/x-icon">
</head>
<body>
<div id="error">
    <div class="error-page container">
        <div class="col-md-8 col-12 offset-md-2">
            <div class="text-center">
                <img class="img-error" src="<?= base_url('assets/images/samples/error-500.svg'); ?>" alt="Not Found">
                <h1 class="error-title">Whoops! System Error</h1>
                <p class="fs-5 text-gray-600">We seem to have hit a snag. Please try again later or contact the developer.</p>
                <a href="<?= base_url('web'); ?>" class="btn btn-lg btn-outline-primary mt-3">Go Home</a>
            </div>
        </div>
    </div>
</div>
</body>
</html>
